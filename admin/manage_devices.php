...
// Validate the lab name to match the pattern "lab_x"
...
$table_name = "{$lab}_devices"; // Dynamically create the table name
...
// Handle form submissions for adding, updating, or deleting devices
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $device_name = trim($_POST['device_name'] ?? '');
    $working_condition = $_POST['working_condition'] ?? 'Good';

    if (isset($_POST['add_device']) && !empty($device_name)) {
        // Insert new device into the selected lab's table
        $stmt = $conn->prepare("INSERT INTO $table_name (device_name, working_condition) VALUES (?, ?)");
        $stmt->bind_param("ss", $device_name, $working_condition);
        $stmt->execute();
    } elseif (isset($_POST['update_device'])) {
        $device_id = $_POST['device_id'];
        // Update existing device
        $stmt = $conn->prepare("UPDATE $table_name SET device_name=?, working_condition=? WHERE id=?");
...
// Fetch all devices from the selected lab's table
$devices = $conn->query("SELECT * FROM $table_name ORDER BY time_added DESC");
?>
