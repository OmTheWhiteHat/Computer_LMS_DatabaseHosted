...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_stock'])) {
        ...
        if ($quantity < 0) {
            ...
            $query = $conn->prepare("INSERT INTO `{$lab}_stocks` (item_name, quantity) VALUES (?, ?)");
            $query->bind_param("si", $item_name, $quantity);
            $query->execute();
        }
        ...
        if ($quantity < 0) {
            ...
            $query = $conn->prepare("UPDATE `{$lab}_stocks` SET item_name = ?, quantity = ? WHERE id = ?");
            $query->bind_param("sii", $item_name, $quantity, $stock_id);
            $query->execute();
        }
    } elseif (isset($_POST['delete_stock'])) {
        $stock_id = $_POST['stock_id'];
        $query = $conn->prepare("DELETE FROM `{$lab}_stocks` WHERE id = ?");
