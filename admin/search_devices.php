...
if (!empty($query)) {
    $stmt = $conn->prepare("SELECT * FROM {$lab}_devices WHERE device_name LIKE ? ORDER BY time_added DESC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    $stmt = $conn->prepare("SELECT * FROM {$lab}_devices ORDER BY time_added DESC");
}
